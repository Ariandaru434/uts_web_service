...
function gotable(nama) {
  ...
  $.ajax({
    url: "http://localhost/php-api-uts/api.php/records/" + nama,
    success: function (data) {
      if (data.records.length > 0) {
        ...
        for (i = 0; i < data.data.length; i++) {
          data.data[i].detil =
            '<button type="button" class="btn btn-primary btn-sm btdetil">Detil</button>';
        }
        if (datatable) {
          datatable.clear();
          datatable.destroy();
...
$("#tabelku").on("click", ".btdetil", function () {
  ...
  for (var i in columnNames) {
    ...
    teks =
      teks +
      '<tr><td class="detlabel">' +
      capitalizeFirstLetter(columnNames[i]) +
      "</td>";
...
$("#modal-detil").on("click", "#modal-save", function (event) {
  ...
  if (dataedit["id"]) {
    for (var i in columnNames) {
      if (
        datasingle[columnNames[i]] === dataedit[columnNames[i]] &&
        columnNames[i] !== "id"
      )
        delete dataedit[columnNames[i]];
    }
  } else {
    for (var i in columnNames) {
      if (dataedit[columnNames[i]] == "" || dataedit[columnNames[i]] == null)
        delete dataedit[columnNames[i]];
    }
  }
  ...
  if (dataedit["id"]) {
    $.ajax({
      url:
        "http://localhost/php-api-uts/api.php/records/" +
        namatable +
        "/" +
        dataedit["id"],
      type: "put",
    ...
    $.ajax({
      url: "http://localhost/php-api-uts/api.php/records/" + namatable,
      type: "post",
      data: dataedit,
      success: function (response) {
        if (response == 1) {
